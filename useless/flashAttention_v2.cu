...
__global__ void kernel_flashAttention(int batch_size, int target_seq_len, int src_seq_len, int q_heads, int kv_heads, int head_dim, bool is_causal, const T* Q, const T* K, const T* V, T* O){
  ...
  const int Br = 16;//Q纵向每块大小
  const int Bc = 16;//K/V纵向分块大小
  ...
  for(int i = 0; i < Tr; ++i){//对于每个Q分块
    ...
    for(int j = 0; j < Tc; ++j){//对于每个K/V分块
      ...
      if(tid_y < bound_tid_y && tid_x < bound_tid_x){//用于边缘不完整块
        ...
        if(is_compute){
          ...
          //                  K[bid_y][Bc * j + tid_x][bid_x / p][k];
            val0 += float(Q[((((bid_y * target_seq_len) + (Br * i + tid_y)) * q_heads) + bid_x) * head_dim + k]) *\
      ...
      if(tid_x == 0 && tid_y < bound_tid_y){
        l_new[tid_y] = myexp<float>(m_prev[tid_y] - m_new[tid_y]) * l_prev[tid_y] + val2;
      }
      __syncthreads();
      ...
      if(tid_x == 0 && tid_y < bound_tid_y){//32路并行计算Oi的每一行
        for(int u = 0; u < head_dim; ++u){
          ...
          O[((((bid_y * target_seq_len) + (Br * i + tid_y)) * q_heads) + bid_x) * head_dim + u] =\
            T(float(O[((((bid_y * target_seq_len) + (Br * i + tid_y)) * q_heads) + bid_x) * head_dim + u]) / myexp<float>(m_prev[tid_y] - m_new[tid_y]) + val3);
        }
      }
      __syncthreads();
    ...
    if(tid_x == 0 && tid_y < bound_tid_y){//32路并行计算Oi的每一行
      for(int u = 0; u < head_dim; ++u){
        O[((((bid_y * target_seq_len) + (Br * i + tid_y)) * q_heads) + bid_x) * head_dim + u] =\
          T(float(O[((((bid_y * target_seq_len) + (Br * i + tid_y)) * q_heads) + bid_x) * head_dim + u]) / l_prev[tid_y]);
      }
    }
    __syncthreads();
