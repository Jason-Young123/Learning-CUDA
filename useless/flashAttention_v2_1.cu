...
__global__ void kernel_flashAttention(int batch_size, int target_seq_len, int src_seq_len, int q_heads, int kv_heads, int head_dim, bool is_causal, const T* Q, const T* K, const T* V, T* O){
  ...
  const int Br = 16;//Q纵向每块大小, 默认为16 (RTX 5090)
  const int Bc = 16;//K/V纵向分块大小, 默认为16
  const int Tr = (target_seq_len + Br - 1) / Br;//对应原始论文中Q纵向分块数Tr,其中Br = 32
  const int Tc = (src_seq_len + Bc - 1) / Bc;//对应原始论文中K/V纵向分块数Tc,其中Bc = 32

  //定义一系列临时变量
  __shared__ T SP[Br][Bc];//复用S和P
  __shared__ T m_prev[Br], m_new[Br];
  __shared__ T l_prev[Br], l_new[Br];

  // 指针类型转换和布局
  /*extern __shared__ char shared_mem[];
  T (*SP)[Bc] = reinterpret_cast<T(*)[Bc]>(shared_mem);
  T* m_prev = reinterpret_cast<T*>(shared_mem + Br * Bc * sizeof(T));
  T* m_new = m_prev + Br;
  T* l_prev = m_new + Br;
  T* l_new = l_prev + Br;*/

  for(int i = 0; i < Tr; ++i){//对于每个Q分块
    int bound_tid_y = min(Br, target_seq_len - Br * i);
    //step0: reset l to 0 and m to -INFINITY
    if(tid_x == 0){
      m_prev[tid_y] = T(-INFINITY);
      l_prev[tid_y] = T(0);
    }
    __syncthreads();
    ...
    for(int j = 0; j < Tc; ++j){//对于每个K/V分块
      ...
      if(tid_y < bound_tid_y && tid_x < bound_tid_x){//用于边缘不完整块
        ...
        if(is_compute){
          ...
          //S[tid_y][tid_x] += Q[bid_y][Br * i + tid_y][bid_x][k] *\
          //                  K[bid_y][Bc * j + tid_x][bid_x / p][k];
            val0 += Q[((((bid_y * target_seq_len) + (Br * i + tid_y)) * q_heads) + bid_x) * head_dim + k] *\
                    K[((((bid_y * src_seq_len) + (Bc * j + tid_x)) * kv_heads) + bid_x / p) * head_dim + k];
          }
          SP[tid_y][tid_x] = val0 / T(sqrtf(head_dim));//缩放因子
        }
      }
      __syncthreads();

      //step2: m_new = max(m_prev, rowMax(S))
      T val1 = SP[tid_y][tid_x];
      ...
      if(tid_y < bound_tid_y && tid_x < bound_tid_x){
        if(is_compute){
          SP[tid_y][tid_x] = myexp<T>(SP[tid_y][tid_x] - m_new[tid_y]);
        }
        else{
          SP[tid_y][tid_x] = T(0);
        }
      ...
      T val2 = SP[tid_y][tid_x];
      val2 = warp_reduce_sum(val2);
      if(tid_x == 0 && tid_y < bound_tid_y){
        l_new[tid_y] = myexp<T>(m_prev[tid_y] - m_new[tid_y]) * l_prev[tid_y] + val2;
      }
      __syncthreads();

      //step5: O = 1/(exp(m_prev - m_new)) * O + P @ V
      if(tid_x < bound_tid_x && tid_y < bound_tid_y){//32路并行计算Oi的每一行
        for(int u = tid_x; u < head_dim; u += 16){
          T val3 = T(0);
          for(int w = 0; w < Bc; ++w){//val2 += P[tid_y][w] * V[bid_y][Bc * j + w][bid_x / p][u];
            val3 += SP[tid_y][w] * V[((((bid_y * src_seq_len) + (Bc * j + w)) * kv_heads) + (bid_x / p)) * head_dim + u];
          }
          //O[bid_y][Br * i + tid_y][bid_x][u] =
          O[((((bid_y * target_seq_len) + (Br * i + tid_y)) * q_heads) + bid_x) * head_dim + u] =\
          O[((((bid_y * target_seq_len) + (Br * i + tid_y)) * q_heads) + bid_x) * head_dim + u] / myexp<T>(m_prev[tid_y] - m_new[tid_y]) + val3;
        }
    ...
    if(tid_x == 0 && tid_y < bound_tid_y){//32路并行计算Oi的每一行
      for(int u = 0; u < head_dim; ++u){
        O[((((bid_y * target_seq_len) + (Br * i + tid_y)) * q_heads) + bid_x) * head_dim + u] =\
          O[((((bid_y * target_seq_len) + (Br * i + tid_y)) * q_heads) + bid_x) * head_dim + u] / l_prev[tid_y];
      }
    }
    __syncthreads();
